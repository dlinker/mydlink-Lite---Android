...
//#endif
		...
		FILE * fp_out = NULL;
		if (cnt == 1){
			snprintf(buf,100,"/sdcard/jni2c_in_%d.yuv",cnt);
			fp_in = fopen(buf,"wb");
			fwrite(Buf,1,nalLen,fp_in);
			fclose(fp_in);
			memset(buf,0,100);
			snprintf(buf,100,"/sdcard/jni2c_out_%d.yuv",cnt);
			fp_out = fopen(buf,"wb");
			//fwrite(Pixel,1,out_pic->linesize[0],fp_out);
			fwrite(Pixel,1,320 * 240 * 1.5,fp_out);
			fclose(fp_out);
		}
