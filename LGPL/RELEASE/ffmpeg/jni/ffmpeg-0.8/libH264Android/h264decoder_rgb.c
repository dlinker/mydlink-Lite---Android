...
//JNIEXPORT jint JNICALL Java_264_com_VView_InitDecoder(JNIEnv* env, jobject thiz, jint width, jint height)
{
  ...
  //  img_convert_ctx = sws_getContext(width, height, c->pix_fmt, width, height,(PixelFormat)PIX_FMT_RGB565,SWS_BICUBIC, NULL, NULL, NULL);	
   #endif
    return -7;
...
//JNIEXPORT jint JNICALL Java_264_com_VView_DecoderNal(JNIEnv* env, jobject thiz, jbyteArray in, jint nalLen, jbyteArray out)
{
	...
	if (nalLen > 0) {
		...
		if (got_picture) {
			...
			{ 
				if(avpicture_alloc((AVPicture *)out_pic, PIX_FMT_RGB565, c->width, c->height)>=0) 
				{ 
					...
					picture->linesize[1] *= -1; 
			   		...
			   		sws_scale(img_convert_ctx, (const uint8_t * const *)picture->data, picture->linesize, 0, c->height, out_pic->data, out_pic->linesize); 
